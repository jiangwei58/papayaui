<template>
  <input
    class="wei-number-input"
    :class="{ disabled }"
    :style="{ textAlign: align }"
    :placeholder="placeholder"
    :value="modelValue"
    :type="precision ? 'digit' : 'number'"
    placeholder-class="wei-number-input-placeholder"
    :disabled="disabled"
    @input="onInput"
    @blur="onBlur"
  />
...
.wei-number-input {
  ...
  color: var(--number-input-text-color, var(--color-primary));
  height: 48rpx;
  line-height: inherit;
  margin: 0;
  min-height: 48rpx;
  ...
  width: 100%;
  &.disabled {
    --number-input-text-color: var(--number-input-disabled, #323233);
  }
  :deep(.wei-number-input-placeholder) {
    color: var(--number-input-placeholder-color, #c8c9cc);
  }
}
</style>
