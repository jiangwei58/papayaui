<template>
  <view class="wei-popover">
    <view v-show="show" class="wei-popover-overlay" @tap="onVisibleChange(false)"></view>
    <view @tap.stop="onVisibleChange()">
      <slot></slot>
    </view>
    <view
      v-show="show"
      class="wei-popover-wapper light bottom"
      :style="{
...
watch(show, (newVal) => {
  if (instance && newVal) {
    nextTick(() => {
      useRect(instance, '.wei-popover').then((res) => {
        tooltipRect.value = res
      })
      useRect(instance, '.wei-popover-wapper').then((res) => {
